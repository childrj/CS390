...
WaveFile::get_sample(int i) {
	short val;
	if (isLittleEndian()) {
		*((unsigned char*)&val + 1) = (hdr->data[2 * i + 1]);//2 * because 2 bytes per sample
...
WaveFile::echo(float echoAmount, float delayms)
{
	...
	while (t < lastSample) {
		...
		w2->add_sample((int)((1 - echoAmount)*value +
...
WaveFile *
WaveFile::get_fragment(double begin, double end) {

	WaveFile * newWave = new WaveFile(numChannels, sampleRate, bitsPerSample);

	for (int i = (begin * (this->sampleRate / 1000)); i < (end * (this->sampleRate / 1000)); i++) {
...
WaveFile::remove_fragment(double begin, double end) {
	...
	for (int a = 0; a < (begin * (this->sampleRate / 1000)); a++) {
		newWave->add_sample(this->get_sample(a));
	}

	for (int a = (end * (this->sampleRate / 1000)); a < this->lastSample; a++) {
		newWave->add_sample(this->get_sample(a));
	}
...
WaveFile *
WaveFile::insert_fragment(int begin, WaveFile * clipboard)
{
	WaveFile * wave = new WaveFile(numChannels, sampleRate, bitsPerSample);
	int start = begin * (this->sampleRate / 1000);
	
	//iterate over samples until reach selection start point
	for (int i = 0; i < start; i++) {
		wave->add_sample(this->get_sample(i));
	}
	//at selection start, add samples from clipboard until clip->lastSample
	for (int i = 0; i < clipboard->lastSample; i++) {
		wave->add_sample(clipboard->get_sample(i));
	}
	//resume copying at end of first loop
	for (int i = start; i < this->lastSample; i++) {
		wave->add_sample(this->get_sample(i));
	}
